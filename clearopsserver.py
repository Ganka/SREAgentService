...
# ═════════════════════════════════════════════════════════════════════════
# KPI tracking — every timestamp below is captured at the exact moment the
# matching broadcast() event already fires elsewhere in the pipeline.
# No new instrumentation needed inside the pipeline functions themselves —
# this module just records each milestone and computes deltas on demand.
# ═════════════════════════════════════════════════════════════════════════
 
KPI_STORE: dict[str, dict] = {}   # error_id -> milestone timestamps + approval waits
KPI_HISTORY: list[dict] = []      # completed incidents, for aggregate stats
BASELINE_MANUAL_MTTR_SEC = 60 * 60  # 60 min, matches the pitch deck baseline
 
 
def kpi_mark(error_id: str, milestone: str):
    """Record the current time against a milestone name for this incident."""
    if error_id not in KPI_STORE:
        KPI_STORE[error_id] = {"milestones": {}, "approval_waits": [], "gates": {}}
    KPI_STORE[error_id]["milestones"][milestone] = datetime.now(timezone.utc)
 
 
def kpi_mark_approval_start(error_id: str, approval_id: str, gate: str):
    if error_id not in KPI_STORE:
        KPI_STORE[error_id] = {"milestones": {}, "approval_waits": [], "gates": {}}
    KPI_STORE[error_id].setdefault("_pending_approval_start", {})[approval_id] = (
        gate, datetime.now(timezone.utc)
    )
 
 
def kpi_mark_approval_end(error_id: str, approval_id: str, approved_or_choice):
    store = KPI_STORE.get(error_id)
    if not store:
        return
    pending = store.get("_pending_approval_start", {})
    if approval_id not in pending:
        return
    gate, started = pending.pop(approval_id)
    waited = (datetime.now(timezone.utc) - started).total_seconds()
    store["approval_waits"].append({"gate": gate, "seconds": round(waited, 2)})
    store["gates"][gate] = approved_or_choice
 
 
def _delta(store: dict, a: str, b: str) -> Optional[float]:
    """Seconds from milestone a to milestone b, or None if either is missing."""
    ms = store.get("milestones", {})
    if a not in ms or b not in ms:
        return None
    return round((ms[b] - ms[a]).total_seconds(), 2)
 
 
def compute_kpis(error_id: str) -> dict:
    """
    Compute the full KPI set for one incident from recorded milestones.
    Every field maps directly to the metrics discussed for the dashboard:
    MTTD, MTTA, MTTR, per-stage latency, human approval wait time,
    auto-diagnosis flag, postmortem (Jira) coverage flag.
    """
    store = KPI_STORE.get(error_id, {"milestones": {}, "approval_waits": [], "gates": {}})
 
    mttd = _delta(store, "fault_occurred", "incident_start") or 0.0
    mtta = _delta(store, "incident_start", "jira_created")
    mttr = _delta(store, "incident_start", "incident_complete")
    detect_to_analysis = _delta(store, "incident_start", "analysis")
    analysis_to_jira = _delta(store, "analysis", "jira_created")
    jira_to_alert = _delta(store, "jira_created", "slack_sent")
    alert_to_fix = _delta(store, "slack_sent", "fix_generated")
    fix_to_complete = _delta(store, "fix_generated", "incident_complete")
 
    total_approval_wait = round(sum(w["seconds"] for w in store["approval_waits"]), 2)
    auto_work_time = round((mttr or 0) - total_approval_wait, 2) if mttr is not None else None
 
    toil_saved_sec = None
    if mttr is not None:
        toil_saved_sec = round(max(0, BASELINE_MANUAL_MTTR_SEC - mttr), 2)
    ...
    return {
        ...
        "mttr_sec": mttr,
        "stage_latency_sec": {
            "detect_to_analysis": detect_to_analysis,
            "analysis_to_jira": analysis_to_jira,
            "jira_to_alert": jira_to_alert,
            "alert_to_fix": alert_to_fix,
            "fix_to_complete": fix_to_complete,
        },
        "human_approval_wait_sec": total_approval_wait,
        "approval_waits_breakdown": store["approval_waits"],
        "auto_work_time_sec": auto_work_time,
        "toil_saved_sec": toil_saved_sec,
        "auto_diagnosis": "analysis" in store.get("milestones", {}),
        "postmortem_logged": "jira_created" in store.get("milestones", {}),
        "gate_outcomes": store["gates"],
    }
 
 
def kpi_finalize(error_id: str) -> dict:
    """Call once at incident_complete — computes final KPIs and archives them."""
    result = compute_kpis(error_id)
    KPI_HISTORY.append(result)
    return result
 
 
def kpi_aggregate() -> dict:
    """Rollup stats across every completed incident this session — for the
    summary cards at the top of the dashboard (avg MTTR, automation rate, etc)."""
    if not KPI_HISTORY:
        return {
            "incident_count": 0, "avg_mttr_sec": None, "avg_mtta_sec": None,
            "avg_human_wait_sec": None, "auto_diagnosis_rate": None,
            "postmortem_coverage": None, "total_toil_saved_sec": 0,
        }
    n = len(KPI_HISTORY)
    mttrs = [k["mttr_sec"] for k in KPI_HISTORY if k["mttr_sec"] is not None]
    mttas = [k["mtta_sec"] for k in KPI_HISTORY if k["mtta_sec"] is not None]
    waits = [k["human_approval_wait_sec"] for k in KPI_HISTORY]
    diag_count = sum(1 for k in KPI_HISTORY if k["auto_diagnosis"])
    post_count = sum(1 for k in KPI_HISTORY if k["postmortem_logged"])
    toil = sum(k["toil_saved_sec"] or 0 for k in KPI_HISTORY)
    return {
        "incident_count": n,
        "avg_mttr_sec": round(sum(mttrs) / len(mttrs), 2) if mttrs else None,
        "avg_mtta_sec": round(sum(mttas) / len(mttas), 2) if mttas else None,
        "avg_human_wait_sec": round(sum(waits) / len(waits), 2) if waits else 0,
        "auto_diagnosis_rate": round(100 * diag_count / n, 1),
        "postmortem_coverage": round(100 * post_count / n, 1),
        "total_toil_saved_sec": round(toil, 2),
    }
...
async def run_phase1(error_payload: dict) -> dict:
    ...
    # KPI: fault_occurred uses the client's own timestamp if it sent one
    # (the browser knows the instant the crash actually happened, which is
    # earlier than when this server received the POST) — falls back to now.
    if error_id not in KPI_STORE:
        KPI_STORE[error_id] = {"milestones": {}, "approval_waits": [], "gates": {}}
    client_ts = error_payload.get("timestamp")
    if client_ts:
        try:
            KPI_STORE[error_id]["milestones"]["fault_occurred"] = datetime.fromisoformat(
                client_ts.replace("Z", "+00:00")
            )
        except ValueError:
            kpi_mark(error_id, "fault_occurred")
    else:
        kpi_mark(error_id, "fault_occurred")
 
    await broadcast({"type": "incident_start", "error_id": error_id, "error_type": error_type,
                     "message": message, "component": component})
    kpi_mark(error_id, "incident_start")
    ...
    for _ in range(8):
        ...
        for tc in msg.tool_calls:
            ...
            if fn == "analyse_error":
                ...
                kpi_mark(error_id, "jira_created")
                ...
                kpi_mark(error_id, "fix_generated")
...
async def run_phase2(state: dict):
    ...
    kpi_mark_approval_start(error_id, approval_id, "push")
    ...
    kpi_mark_approval_end(error_id, approval_id, decision.get("approved", False))
    ...
    if pr_info:
        ...
        kpi_mark_approval_start(error_id, approval_id2, "merge")
        ...
        kpi_mark_approval_end(error_id, approval_id2, decision2.get("approved", False))
    ...
    if jira_key:
        ...
        kpi_mark_approval_start(error_id, approval_id3, "jira")
        ...
        kpi_mark_approval_end(error_id, approval_id3, decision3.get("choice", "Skip"))
    ...
    kpi_mark(error_id, "incident_complete")
    ...
    incident_kpis = kpi_finalize(error_id)
    await broadcast({"type": "kpi_update", "incident": incident_kpis, "aggregate": kpi_aggregate()})
...
@app.get("/kpis")
async def get_kpis():
    """Returns the rollup KPI stats plus a per-incident breakdown — useful
    for plugging into an external dashboard or just curling during a demo."""
    return JSONResponse({
        "aggregate": kpi_aggregate(),
        "incidents": KPI_HISTORY,
    })
...
<style>
  @import url('https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@400;500;600&family=Inter:wght@400;500;600;700&display=swap%27);
  ...
  * { box-sizing: border-box; }
  ...
  .kpi-empty { color: var(--muted); font-size: 12.5px; text-align: center; padding: 16px 0; }
</style>
</head>
<body>
<div class="layout">
  ...
  <div class="side-col">
    <div class="kpi-panel">
      <div class="side-header" style="padding:0 0 12px;border:none;">SRE metrics</div>
      <div class="kpi-grid" id="kpi-grid">
        <div class="kpi-empty" style="grid-column:1/-1;">No incidents completed yet.</div>
      </div>
    </div>
...
fetch('/kpis').then(r => r.json()).then(data => {
  if (data.aggregate && data.aggregate.incident_count > 0) renderKpis(data.aggregate, null);
}).catch(() => {});
...
function handleEvent(ev) {
  ...
  switch (ev.type) {
    ...
    case 'kpi_update':
      renderKpis(ev.aggregate, ev.incident);
      break;
  }
}
...
function renderKpis(agg, lastIncident) {
  const grid = document.getElementById('kpi-grid');
  if (!agg || agg.incident_count === 0) {
    grid.innerHTML = '<div class="kpi-empty" style="grid-column:1/-1;">No incidents completed yet.</div>';
    return;
  }
  const cards = [
    { label: 'MTTR (avg)', value: fmtSec(agg.avg_mttr_sec), cls: 'coral', sub: `${agg.incident_count} incident${agg.incident_count > 1 ? 's' : ''}` },
    { label: 'MTTA (avg)', value: fmtSec(agg.avg_mtta_sec), cls: 'teal', sub: 'detect to Jira' },
    { label: 'Human wait (avg)', value: fmtSec(agg.avg_human_wait_sec), cls: '', sub: 'approval gates' },
    { label: 'Toil saved', value: fmtSec(agg.total_toil_saved_sec), cls: 'teal', sub: 'vs 60min manual baseline' },
    { label: 'Auto-diagnosis', value: agg.auto_diagnosis_rate + '%', cls: '', sub: 'root cause found by AI' },
    { label: 'Postmortem coverage', value: agg.postmortem_coverage + '%', cls: '', sub: 'Jira story per incident' },
  ];
